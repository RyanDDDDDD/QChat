    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    this->setModal(true);
    ui->name_ed->setPlaceholderText(tr("llfc"));
    ui->lb_ed->setPlaceholderText("search, add label");
    ui->back_ed->setPlaceholderText("User");
    ...
    _tip_data = { "classmate","family", "C++ Primer", "Python"};

    connect(ui->more_lb, &ClickedOnceLabel::clicked, this, &ApplyFriend::ShowMoreLabel);
    InitTipLbs();

    connect(ui->lb_ed, &CustomizedEdit::returnPressed, this, &ApplyFriend::SlotLabelEnter);
    connect(ui->lb_ed, &CustomizedEdit::textChanged, this, &ApplyFriend::SlotLabelTextChange);
    connect(ui->lb_ed, &CustomizedEdit::editingFinished, this, &ApplyFriend::SlotLabelEditFinished);
...
void ApplyFriend::SlotLabelEnter()
{
    ...
    if (find_add != _add_labels.end()) {
        find_add.value()->SetCurState(ClickedState::Selected);
        return;
    }
    ...
    lb->SetCurState(ClickedState::Selected);
...
void ApplyFriend::SlotChangeFriendLabelByTip(QString lbtext, ClickedState state)
{
    ...
    if(state == ClickedState::Selected){

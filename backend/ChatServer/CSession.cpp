...
void CSession::AsyncReadBody(int total_len)
{
	...
	asyncReadFull(total_len, [self, this, total_len](const boost::system::error_code& ec, std::size_t bytes_transfered) {
		try {
			...
			//post msg to logic queue to be processed
...
void CSession::AsyncReadHead(int total_len)
{
	...
	asyncReadFull(HEAD_TOTAL_LEN, [self, this](const boost::system::error_code& ec, std::size_t bytes_transfered) {
		try {
			...
			//get header msg id data
			short msg_id = 0;
			memcpy(&msg_id, _recv_head_node->_data, HEAD_ID_LEN);
			//convert network msg from big Endianness to local Endianness
	...
	std::function<void(const boost::system::error_code&, std::size_t)> handler)
{
	...
	_socket.async_read_some(boost::asio::buffer(_data + read_len, total_len - read_len),
		[read_len, total_len, handler, self](const boost::system::error_code& ec, std::size_t  bytesTransfered) {
			if (ec) {
				//error occurs, run callback
				handler(ec, read_len + bytesTransfered);
				return;
			}

			if (read_len + bytesTransfered >= total_len) {
				//read enough msg, run callback
				handler(ec, read_len + bytesTransfered);
				return;
			}

			//no error but length is not enough, keep reading

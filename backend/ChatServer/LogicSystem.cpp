...
void LogicSystem::PostMsgToQue(shared_ptr < LogicNode> msg) {
	...
	_msg_que.push(msg);
	//notify when msg queue size is 1
	if (_msg_que.size() == 1) {
		unique_lk.unlock();
		_consume.notify_one();
	}
...
void LogicSystem::DealMsg() {
	while (true) {
		std::unique_lock<std::mutex> unique_lk(_mutex);
		//block thread when queue is empty by condition variable
		while (_msg_que.empty() && !_b_stop) {
			_consume.wait(unique_lk);
		}

		//if system is stop, run all remainning tasks and exit
		if (_b_stop) {
		...
		//system is not stopped, and queue is not empty
		auto msg_node = _msg_que.front();
		std::cout << "recv_msg id  is " << msg_node->_recvnode->_msg_id << endl;
		auto call_back_iter = _fun_callbacks.find(msg_node->_recvnode->_msg_id);
		if (call_back_iter == _fun_callbacks.end()) {
...
void LogicSystem::LoginHandler(shared_ptr<CSession> session, const short& msg_id, const string& msg_data) {
	...
	//increment login count in redis
